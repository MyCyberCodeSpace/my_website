...
def details_portfolio(request, slug):
    if request.method == 'GET':
        arquivo_path = f'{settings.BASE_DIR}/templates/apresentacao/portfolio/portfolio_detalhado_en/{slug}.html'
        if os.path.exists(arquivo_path):  
            with open(arquivo_path, 'r') as arquivo:
                conteudo = arquivo.read()
            formatted_slug = slug.replace('_', ' ').title()
            return render(request, 'apresentacao/portfolio/05_detalhes_portifolio_en.html',{'conteudo': conteudo, 'indicativo': formatted_slug})
        else:
            typeErroMessage = 'ATTEMPT INVVALID SLUG GET ON PORTFOLIO - PORTFOLIO DETAILS'
            return attemtHacking(request, typeErroMessage)
    else:
        typeErroMessage = 'ATTEMPT TO POST IN PROHIBITED LOCATION - PORTFOLIO DETAILS'
        return attemtHacking(request, typeErroMessage)
